...
def run_tuned_pls(
    ...
    """PLSRegression with Optuna nested CV.

      1. Inner CV (4×5=20) + Optuna (20 trials) tunes n_components on X_train.
...
def run_tuned_pls_mrmr(
    df_combined, K=11, target_col='pain_reduction', random_state=42,
    target_transformer=None,
):
    """PLSRegression with MRMR feature selection (K features) inside each outer CV fold.

      1. MRMR on X_train → select K features per outer fold
      2. Inner CV (4×5=20) + Optuna (20 trials) tunes n_components on X_train.
    ...
    print(f"\n{'='*65}")
    print(f"  PLS + MRMR (K={K}) + Optuna — {target_col}")
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(X), start=1):
        ...
        # NOTE: n_comp_upper bounded by number of selected features per fold
        n_comp_upper = min(len(selected_cols), 10)
        inner_splits = list(inner_cv.split(X_train_scaled))

...
# with mrmr inside cv
def _encode_for_final(X, cat_cols):
    out  = X.copy()
...
def run_tuned_elasticnet_mrmr(
    df_combined, K=11, target_col='pain_reduction', random_state=42,
    target_transformer=None,
):
    """ElasticNet with MRMR feature selection (K features) inside each outer CV fold.

      1. MRMR on X_train → select K features per outer fold
      2. Inner CV (4×5=20) + Optuna (50 trials) tunes ElasticNet hyperparameters on X_train
      3. Train final fold model on X_train → evaluate on X_test
      4. Final model: features selected in ≥50% of outer folds, median HPs across outer folds
    ...
    # All features — MRMR selects K per fold
    ...
    print(f"\n{'='*65}")
    print(f"  ElasticNet + MRMR (K={K}) + Optuna — {target_col}")
    ...
    for feat, cnt in feature_freq[feature_freq > 0].items():
        marker = '  (≥50%)' if cnt / n_outer >= 0.50 else ''

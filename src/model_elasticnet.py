...
def run_advanced_elasticnet_rent(
    ...
    """ElasticNet with Optuna-tuned RENT and Model Hyperparameters.
      1. Tune RENT HPs via Optuna on 75-25 split of imputed X_train
      2. Re-run RENT on full imputed X_train with best HPs
    ...
    ElasticNet requires imputation + scaling. StandardScaler fitted on
    X_train only, applied to X_test
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(X), start=1):
        ...
        # ── Scale X_train for ElasticNet — fit on train only ─────────────────
        scaler          = StandardScaler()
        X_train_scaled  = pd.DataFrame(
            scaler.fit_transform(X_train_rent),
            columns=feature_cols, index=X_train_rent.index)
        scalers_per_fold.append(scaler)

        # ── Step 1: Tune RENT HPs on 75-25 split ─────────────────────────────
        X_tr_rent, X_val_rent, y_tr, y_val = train_test_split(
            X_train_scaled, y_train_fit, test_size=0.25, random_state=random_state)
        ...
        def rent_objective(trial):
            ...
            # Probe ElasticNet (instead of CatBoost) to score selected features
            probe = ElasticNet(
        ...
        # Use scaled, imputed data — ElasticNet cannot handle NaN
        ...
        # Impute + encode + scale X_test using SAME scaler fitted on X_train
        X_test_imp = imputer.transform(X_test)
        X_test_imp = pd.DataFrame(X_test_imp, columns=X_test.columns, index=X_test.index)

        X_test_rent   = _prep_for_rent(X_test_imp, cat_cols)
        
        X_test_scaled = pd.DataFrame(
            scaler.transform(X_test_rent),       # transform only
            columns=feature_cols, index=X_test_rent.index)
    ...
    if not [f for f, cnt in freq.items() if cnt / n_outer >= 0.75]:
        print("   No features met ≥75% threshold — "
    ...
    # Fit final scaler + imputer on full dataset
    X_final_imp, _ = preprocess.impute_iterative(
        X, ex_cols=None, iterations=10, random_state=42, verbose=False)
    X_final_rent   = _prep_for_rent(X_final_imp, cat_cols)
    scaler_final   = StandardScaler()
    X_final_scaled = pd.DataFrame(
        scaler_final.fit_transform(X_final_rent),
        columns=feature_cols, index=X_final_rent.index)

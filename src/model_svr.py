...
def svr_mrmr(
    ...
    """SVR (RBF) with MRMR (K + RFCQ params tuned by Optuna) inside each outer CV fold.
      ...
      3. Inner CV (4×5=20) + Optuna (50 trials) tunes SVR hyperparameters (C, epsilon, gamma).
    ...
    p = len(feature_cols)
    print(f"\n{'='*80}")
    print(f" Nested CV - SVR + MRMR + Optuna — {target_col}")
    print(f"  n={len(X)}, p={p}")
...
#_________________________________________________________________________________________________
# SVR Feature-Threshold Analysis
#_________________________________________________________________________________________________

def svr_threshold_analysis(
    df_combined, feature_freq, target_col='pain_reduction_pct',
    random_state=42, target_transformer=None):
    """SVR (RBF) + Optuna nested CV across feature-frequency threshold subsets.
    ...
    choose a feature threshold, then pass the chosen feature list to run_tuned_svr.
    ...
    for step in steps:
        ...
        for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(X), start=1):
            ...
            fold_model = SVR(kernel='rbf', **best_params)
...
def run_tuned_svr(
    ...
    """SVR (RBF) + Optuna nested CV on a fixed feature list.
      1. Inner CV (4×5=20) + Optuna (50 trials) tunes C, epsilon, gamma
